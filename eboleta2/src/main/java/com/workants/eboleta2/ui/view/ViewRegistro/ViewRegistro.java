...
public class ViewRegistro extends VerticalLayout implements IViewRegistro, ValueChangeListener, ClickListener{
	...
	private ViewRegistroEncabezado viewRegistroEncabezado;
	private ViewRegistroCuerpo viewRegistroCuerpo;
	
	public static final String NAME = "viewRegistro";
	
	
	public static Logger log = Logger.getLogger(ViewRegistro.class);
	...
	private Component generarEncabezado() {
		...
		this.setViewRegistroEncabezado(new ViewRegistroEncabezado());
	...
	private Component generarCuerpo() {
		...
		this.setViewRegistroCuerpo(new ViewRegistroCuerpo());
	...
	public ViewRegistroEncabezado getViewRegistroEncabezado() {
	...
	public void setViewRegistroEncabezado(
			ViewRegistroEncabezado viewRegistroEncabezado) {
		this.viewRegistroEncabezado = viewRegistroEncabezado;
	}



	public ViewRegistroCuerpo getViewRegistroCuerpo() {
		return viewRegistroCuerpo;
	}



	public void setViewRegistroCuerpo(ViewRegistroCuerpo viewRegistroCuerpo) {
	...
	public void enter(ViewChangeEvent event) {
		...
		this.getViewRegistroEncabezado().getLblEncabezado().setValue("<strong>Datos a completar del Contribuyente/Solicitante<br>Seleccion:</strong> "+ nombreImpuesto);
	...
	public void suscribirExitoso() {
		...
		//Notification notificacion = new Notification("Atención","Verifique su correo electronico para finalizar el proceso de alta de impuestos.  <a href='http://www.comodoro.gov/eboleta/'>Volver a e-boleta</a> ", Type.ASSISTIVE_NOTIFICATION,true);
		  Notification notificacion = new Notification("Atención","Verifique su correo electronico para completar el proceso de suscripción. "
		    		...
		    		+ "<br><b>Si el proceso no se completa en las siguientes 24 hs, se cancelará y deberó realizar una nueva suscripción  "
		...
		getHandler().limpiar();
		getViewRegistroCuerpo().getLayoutEsTitular().getBtnVolver().setEnabled(true);
		getViewRegistroCuerpo().getLayoutEsTitular().getBtnVolver().setVisible(true);
		getViewRegistroCuerpo().getLayoutEsTitular().getBtnAceptar().setVisible(false);
		
	
		
		//UI.getCurrent().getUI().getPage().setLocation("http://www.comodoro.gov.ar/eboleta2");
...
//		    		+ "Si el proceso no se completa en las siguientes 24 hs, se cancelaró y deberó realizar una nueva suscripción  <a href='http://www.comodoro.gov/eboleta/'>Volver a e-boleta</a> ", Type.ASSISTIVE_NOTIFICATION,true);
	...
	public void subscribirSolicitanteErroneo() {
			...
			UI.getCurrent().getUI().getPage().setLocation(ViewPrincipal.NAME); //redireccionamos
			//Notification notificacion = new Notification("Atención","Verifique su correo electronico para finalizar el proceso de alta de impuestos  <a href='http://www.comodoro.gov/eboleta'>Volver a e-boleta</a> ", Type.ASSISTIVE_NOTIFICATION,true);
			Notification notificacion = new Notification("Atención","Error al suscribir Titular-Solicitante al impuesto. Reintente nuevamente  <a href='http://www.comodoro.gov.ar/eboleta2/'>Volver a e-boleta</a>", Type.ERROR_MESSAGE);
			notificacion.setPosition(Position.MIDDLE_CENTER);
			notificacion.setDelayMsec(3000);
			notificacion.show(Page.getCurrent());
